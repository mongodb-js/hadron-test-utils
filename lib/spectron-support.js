...
function startApplication(distDir) {
  this.app = createApplication(distDir);
  return app.start()
    .then(() => {
 ...
 *
 */
function.addCommands = function(client) {

  /**
   * Fills out the connect form.
   */
  client.addCommand('fillOutForm', (model) => {
    ...
    _.each(staticFields, function(field) {
      if (model[field]) {
        sequence = sequence.then(() => {
          return this.setValue(format('input[name=%s]', field), model[field]);
        });
      }
    });
    ...
    if (model.authentication && model.authentication !== 'NONE') {
      sequence = sequence.then(() => {
        return this.selectByValue('select[name=authentication]', model.authentication);
      });
      var authFields = Connection.getFieldNames(model.authentication);
      _.each(authFields, (field) => {
        if (model[field]) {
          sequence = sequence.then(() => {
            return this.setValue(format('input[name=%s]', field), model[field]);
          });
    ...
    if (model.ssl && model.ssl !== 'NONE') {
      sequence = sequence.then(() => {
        return this.selectByValue('select[name=ssl]', model.ssl);
      });
      var sslFields = ['ssl_ca', 'ssl_certificate', 'ssl_private_key',
        'ssl_private_key_password'];
      _.each(sslFields, (field) => {
        if (model[field]) {
          sequence = sequence.then(() => {
            return this.setValue(format('input[name=%s]', field), model[field]);
          });
